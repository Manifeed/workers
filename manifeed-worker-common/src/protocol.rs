...
pub struct WorkerTaskCompleteRequest<T> {
    pub session_id: String,
    pub lease_id: String,
    pub trace_id: String,
    pub task_type: String,
...
pub fn sign_payload(secret: &str, payload: &Value, mode: CanonicalJsonMode) -> Result<String> {
    let canonical_payload = canonical_json(payload, mode)?;
    let mut mac = HmacSha256::new_from_slice(secret.as_bytes())
        .map_err(|error| WorkerError::Crypto(format!("invalid hmac secret: {error}")))?;
    mac.update(canonical_payload.as_bytes());
    Ok(hex::encode(mac.finalize().into_bytes()))
}

pub fn canonical_json(payload: &Value, mode: CanonicalJsonMode) -> Result<String> {
    serialize_json_ascii(&normalize_json_value(payload), mode)
}
...
fn serialize_json_ascii(value: &Value, mode: CanonicalJsonMode) -> Result<String> {
    match value {
        Value::Null => Ok("null".to_string()),
        Value::Bool(boolean) => Ok(boolean.to_string()),
        Value::Number(number) => match mode {
            CanonicalJsonMode::PreserveNumberFormatting => Ok(number.to_string()),
            CanonicalJsonMode::NormalizeExponentSign => {
                Ok(normalize_json_number(number.to_string()))
            }
        },
        ...
        Value::Array(items) => {
            ...
            for item in items {
                serialized_items.push(serialize_json_ascii(item, mode)?);
            }
        ...
        Value::Object(object) => {
            ...
            for key in keys {
                let item_value = object.get(key).ok_or_else(|| {
                    WorkerError::Json(serde_json::Error::io(std::io::Error::other(
                        "missing object entry during canonical json serialization",
                    )))
                })?;
                serialized_entries.push(format!(
                    "{}:{}",
                    escape_json_ascii_string(key),
                    serialize_json_ascii(item_value, mode)?,
                ));
            }
...
mod tests {
    ...
    use super::{canonical_json, sign_payload, CanonicalJsonMode};
    ...
    fn canonical_json_escapes_unicode_as_ascii() {
        ...
        let canonical = canonical_json(&payload, CanonicalJsonMode::PreserveNumberFormatting)
    ...
    #[test]
    fn canonical_json_normalizes_embedding_scientific_notation() {
        let payload = serde_json::from_str::<Value>(r#"{"vector":[1e-7,1e20,3.5]}"#)
            .expect("payload should parse");

        let canonical = canonical_json(&payload, CanonicalJsonMode::NormalizeExponentSign)
            .expect("canonical json should serialize");

        assert_eq!(canonical, r#"{"vector":[1e-07,1e+20,3.5]}"#);
    }

    #[test]
    fn sign_payload_uses_selected_number_formatting_mode() {
        let payload =
            serde_json::from_str::<Value>(r#"{"vector":[1e-7]}"#).expect("payload should parse");

        let rss_signature = sign_payload(
            "shared-secret",
            &payload,
            CanonicalJsonMode::PreserveNumberFormatting,
        )
        .expect("rss signature should be produced");
        let embedding_signature = sign_payload(
            "shared-secret",
            &payload,
            CanonicalJsonMode::NormalizeExponentSign,
        )
        .expect("embedding signature should be produced");

        assert_ne!(rss_signature, embedding_signature);
    }
}
