...
impl ControlApp {
	...
	pub fn refresh_release_statuses(&mut self) {
		...
		self.embedding.release_status =
			self.compute_release_status(WorkerType::SourceEmbedding);
	}

	pub(crate) fn compute_release_status(
		&self,
		wt: WorkerType,
	...
	pub fn start_worker(&mut self, wt: WorkerType) {
			...
			&& self.config.embedding.acceleration_mode == AccelerationMode::Gpu
		{
			...
			if !gpu.is_supported() {
				self.last_error =
					Some(format!("GPU indisponible: {}", gpu.summary()));
				return;
			}
		}
		...
		if self.service_mode(wt) == ServiceMode::Background {
			match start_user_service(wt) {
				Ok(()) => {
					self.state_mut(wt).last_message =
						Some("service demarre".to_string());
					self.last_error = None;
				}
		...
		if !binary.exists() {
			self.last_error =
				Some(format!("binaire introuvable: {}", binary.display()));
			return;
		}
	...
	pub fn install_service(&mut self, wt: WorkerType) {
		let Some(binary) = self.binary_path(wt) else {
			self.last_error =
				Some(format!("aucun binaire pour {}", wt.display_name()));
			return;
		};
		match install_user_service(wt, &binary, &self.config_path) {
			Ok(()) => {
				...
				self.save_config();
				self.state_mut(wt).last_message =
					Some("service installe".to_string());
			}
			Err(e) => self.last_error = Some(e.to_string()),
		}
	}
	...
	pub fn uninstall_service(&mut self, wt: WorkerType) {
		match uninstall_user_service(wt) {
			Ok(()) => {
				...
				self.save_config();
				self.state_mut(wt).last_message =
					Some("service supprime".to_string());
			}
			Err(e) => self.last_error = Some(e.to_string()),
		}
	}
