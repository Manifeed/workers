...
use manifeed_worker_common::{
	AccelerationMode, ReleaseCheckStatus, ServiceMode, WorkerType,
};
			...
			.show(ui, |ui| {
				...
				ui.horizontal_wrapped(|ui| {
					...
					if ui
						.add_enabled(
							!blocked,
							widgets::secondary_button("Redemarrer"),
						)
						.clicked()
					{
					...
					if ui
						.add(widgets::secondary_button("Ouvrir logs"))
						.clicked()
					{
						self.open_logs(wt);
					}
			...
			|ui| {
				...
				ui.horizontal_wrapped(|ui| {
					...
					widgets::pill(
						ui,
						color,
						widgets::service_mode_label(mode),
					);
				});
				ui.add_space(8.0);
				ui.horizontal_wrapped(|ui| {
					if ui
						.add(widgets::secondary_button("Installer"))
						.clicked()
					{
						self.install_service(wt);
					}
					if ui
						.add(widgets::secondary_button("Supprimer"))
						.clicked()
					{
					...
					if ui
						...
						.clicked()
					{
						self.state_mut(wt).release_status =
							self.compute_release_status(wt);
					}
				});
				ui.add_space(4.0);
				ui.horizontal_wrapped(|ui| {
					if wt == WorkerType::SourceEmbedding
						&& ui
							.add(widgets::secondary_button("Probe GPU"))
							.clicked()
					{
						self.refresh_gpu_support();
					}
					if let Some(release) =
						&self.state(wt).release_status.clone()
					{
						if let Some(manifest) = &release.manifest {
							let url = manifest.download_url.clone();
							if ui
								.add(widgets::secondary_button("Mise a jour"))
								.clicked()
							{
...
fn release_banner_message(
	...
	match r.status {
		...
		ReleaseCheckStatus::UpdateAvailable => {
			Some(format!("{label}: mise a jour disponible."))
		}
		ReleaseCheckStatus::Incompatible => {
			Some(format!("{label}: version locale incompatible."))
		}
		ReleaseCheckStatus::Unverified => {
			r.message.as_ref().map(|m| format!("{label}: {m}"))
		}
	}
}
