...
impl WorkerGatewayClient {
        ...
        F: FnMut(WorkerLeaseRead) -> Result<(u64, u64, T)>,
    {
        ...
        Err(WorkerError::Process("gateway retries exhausted".to_string()).into())
    }
        ...
        F: FnMut(WorkerLeaseRead) -> Result<(u64, u64, T)>,
    {
        ...
        let leases = self
            ...
            .post_json::<_, Vec<WorkerLeaseRead>>(
                ...
                Some(self.authenticator.bearer_token()),
            )
            .await?;
    ...
    fn current_session(&self) -> Result<Option<GatewaySessionState>> {
        self.session
            ...
            .map_err(|_| WorkerError::Process("session mutex poisoned".to_string()).into())
    }
    ...
    fn lock_lease_metadata(
        ...
        self.lease_metadata
            .lock()
            .map_err(|_| WorkerError::Process("lease metadata mutex poisoned".to_string()).into())
    }
}

fn is_retryable_network_error(error: &crate::error::RssWorkerError) -> bool {
    matches!(
        error,
        crate::error::RssWorkerError::Worker(WorkerError::Http(_))
    ) || matches!(error, crate::error::RssWorkerError::Http(_))
}
