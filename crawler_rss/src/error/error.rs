...
impl RssWorkerError {
    pub fn is_auth_error(&self) -> bool {
        matches!(self, Self::Worker(error) if is_auth_error(error))
    }

    pub fn user_facing_message(&self) -> String {
        match self {
            Self::Worker(error) => user_facing_error_message(error),
            Self::Io(_) => "Worker file error".to_string(),
            Self::Json(_) => "Invalid config data".to_string(),
