...
impl HttpFeedFetcher {
    ...
    async fn fetch_once(
        ...
        let parsed_feed = parse_feed_bytes(&bytes, &feed.feed_url).map_err(|error| {
            FetchAttemptError::permanent(Some(response_status.as_u16()), error.to_string())
        })?;
        stdout_log(format!("parsing {} ok {}", feed.feed_id, feed.feed_url));
        let normalized_sources = drop_future_published_sources(
            normalize_sources(&parsed_feed, feed.last_db_article_published_at),
            self.clock.now(),
        );
