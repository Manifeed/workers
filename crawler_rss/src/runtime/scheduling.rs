...
use crate::fetch::scheduling_host_key;
...
mod tests {
    ...
    use super::ClaimedFeedQueue;
    use crate::model::{ClaimedRssTask, RawFeedScrapeResult, RssFeedPayload};

    fn feed(feed_id: u64, url: &str) -> RssFeedPayload {
        RssFeedPayload {
            feed_id,
    ...
    fn task(feeds: Vec<RssFeedPayload>) -> ClaimedRssTask {
        ClaimedRssTask {
            task_id: 1,
            execution_id: 1,
    ...
    #[test]
    fn scheduling_respects_per_host_limit() {
        let mut queue = ClaimedFeedQueue::new(1);
        queue.enqueue_task(task(vec![
            feed(1, "https://example.test/a.xml"),
            feed(2, "https://example.test/b.xml"),
            feed(3, "https://other.test/c.xml"),
        ]));

        let first = queue.try_start_next().unwrap();
        let second = queue.try_start_next().unwrap();
    ...
    #[test]
    fn completed_feed_can_release_next_same_host_feed() {
        let mut queue = ClaimedFeedQueue::new(1);
        queue.enqueue_task(task(vec![
            feed(1, "https://example.test/a.xml"),
            feed(2, "https://example.test/b.xml"),
        ...
        let result =
            RawFeedScrapeResult::not_modified("job-1", true, &first.feed, 304, None, None, 1);
        queue
            .finish_feed(super::CompletedFeed {
                scheduled_feed: first,
                result,
            })
            .unwrap();

        assert!(queue.try_start_next().is_some());
    }
}
