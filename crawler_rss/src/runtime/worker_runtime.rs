...
use crate::pipeline::execute_feed_micro_task;
use crate::ports::{FeedFetcher, RssGateway};
...
pub struct RssWorker<G, F> {
    ...
    last_requested_state: Option<RssGatewayState>,
}
    ...
    pub async fn run_once(&mut self) -> Result<bool> {
        ...
        loop {
            ...
            let worker_event = tokio::select! {
                Some(joined) = fetch_join_set.join_next() => WorkerEvent::Feed(Box::new(
...
{
    fetch_join_set
        .spawn(async move { execute_feed_micro_task(fetcher.as_ref(), scheduled_feed).await });
}
...
async fn request_state<G>(
    last_requested_state: &mut Option<RssGatewayState>,
    gateway: &G,
    state: RssGatewayState,
) where
    G: RssGateway + Clone + Send + Sync + 'static,
{
...
fn matches_reporting_state(
    previous_state: Option<&RssGatewayState>,
    state: &RssGatewayState,
) -> bool {
