...
use manifeed_worker_common::{
    app_paths, check_worker_release_status, check_worker_release_status_with_runtime,
    save_workers_config, WorkerPhase, WorkerReleaseStatus, WorkerStatusSnapshot, WorkerType,
};
...
impl AppCore {
    ...
    pub(super) fn refresh_status(&mut self, worker_type: WorkerType) {
        ...
        self.reconcile_installed_version(worker_type);
    }
    ...
    fn reconcile_installed_version(&mut self, worker_type: WorkerType) {
        if !self.is_installed(worker_type) {
            return;
        }

        let Some(snapshot_version) = self
            .state(worker_type)
            .status_snapshot
            .as_ref()
            .map(|snapshot| snapshot.app_version.trim())
            .filter(|version| !version.is_empty())
        else {
            return;
        };

        let current_version = installed_version(&self.config, worker_type);
        ...
        match worker_type {
            WorkerType::RssScrapper => {
                self.config.rss.installed_version = snapshot_version.to_string();
            }
            WorkerType::SourceEmbedding => {
                self.config.embedding.installed_version = snapshot_version.to_string();
            }
        }

        if let Some(config_path) = self.config_path.clone() {
            let _ = save_workers_config(&config_path, &self.config);
        }
    }
}
